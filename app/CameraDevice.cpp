...
namespace cli
{
    CameraDevice::CameraDevice(std::int32_t no, SCRSDK::ICrCameraObjectInfo const *camera_info)
        : m_number(no), m_device_handle(0), m_connected(false), m_conn_type(ConnectionType::UNKNOWN), m_prop(), m_lvEnbSet(true), m_modeSDK(SCRSDK::CrSdkControlMode_Remote), m_spontaneous_disconnection(false), m_fingerprint(""), m_userPassword(""), m_bodySerialNumberProp(nullptr), m_lensModelNameProp(nullptr), m_recordingSettingFileNameProp(nullptr), m_modelNameProp(nullptr), m_media_formatComplete(false), m_getContentsDataMovieFlg(false), m_getContentsDataStartFlg(false), m_getContentsData_notify(0), m_getContentsData_per(0), m_latestFirmwareUploadRate(0)
    {
    ...
    bool CameraDevice::connect(SCRSDK::CrSdkControlMode openMode, SCRSDK::CrReconnectingSet reconnect)
    {
        ...
        if (SDK::CrSSHsupportValue::CrSSHsupport_ON == get_sshsupport())
        {
            ...
            if (!is_setpassword())
            {
                ...
                while (true)
                {
                    ...
                    else if (ch_ipt == Password_Key_Back && userPw.length() != 0)
                    {
                        userPw.pop_back();
        ...
        if (CR_FAILED(connect_status))
        {
            text id(this->get_id());
            tout << std::endl
                 << "Failed to connect: 0x" << std::hex << connect_status << std::dec << ". " << m_info->GetModel() << " (" << id.data() << ")\n";
    ...
    {
        SDK::CrInt16u value = 0;
        switch (index)
        {
        case 0: value = SDK::CrStillImageStoreDestination::CrStillImageStoreDestination_HostPC; break;
        case 1: value = SDK::CrStillImageStoreDestination::CrStillImageStoreDestination_MemoryCard; break;
        case 2: value = SDK::CrStillImageStoreDestination::CrStillImageStoreDestination_HostPCAndMemoryCard; break;
    ...
    bool CameraDevice::set_save_info() const
    {
        ...
        auto save_status = SDK::SetSaveInfo(m_device_handle, path, (char *)"", ImageSaveAutoStartNo);
#else
        text path = fs::current_path().native();
        tout << path.data() << '\n';

        auto save_status = SDK::SetSaveInfo(m_device_handle, const_cast<text_char *>(path.data()), const_cast<text_char *>(TEXT("")), ImageSaveAutoStartNo);
#endif
    ...
    void CameraDevice::set_focus_mode_new(const std::wstring &label, int index)
    {
        if (1 != m_prop.focus_mode.writable)
        {
            tout << "Focus Mode is not writable\n";
        ...
        if (index < 0 || static_cast<std::size_t>(index) >= values.size())
        {
            tout << "Invalid focus mode index: " << index << '\n';
            return;
        }

        tout << "Setting Focus Mode to [" << index << "] " << format_focus_mode(values[index]) << '\n';
    ...
    void CameraDevice::set_white_balance()
    {
        ...
        text input;
        tout << std::endl
             << "Would you like to set a new White Balance value? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip setting a new value.\n";
        ...
        tout << std::endl
             << "Choose a number set a new White Balance value:\n";
        tout << "[-1] Cancel input\n";
    ...
    void CameraDevice::execute_lock_property(CrInt16u code)
    {
        ...
        text input;
        tout << std::endl
             << "Would you like to execute Unlock or Lock? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
    ...
    void CameraDevice::set_af_area_position()
    {
        ...
        if (1 == m_prop.position_key_setting.writable)
        {
            // Set, PriorityKeySettings property
            tout << std::endl
                 << "Set camera to PC remote" << std::endl;
            SDK::CrDeviceProperty priority;
            priority.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_PriorityKeySettings);
            priority.SetCurrentValue(SDK::CrPriorityKeySettings::CrPriorityKey_PCRemote);
            priority.SetValueType(SDK::CrDataType::CrDataType_UInt32Array);
        ...
        tout << std::endl
             << "Set the Exposure Program mode to P Auto" << std::endl;
        ...
        tout << std::endl
             << "Set Focus Area to Flexible_Spot_S\n";
    ...
    void CameraDevice::set_select_media_format()
    {
            ...
            (SDK::CrMediaFormat::CrMediaFormat_Disable == m_prop.media_slot2_full_format_enable_status.current))
        {
            // Not a settable property
            tout << std::endl
                 << "Slot1 and Slot2 can not format\n";
            return;
        }
        ...
        text input;
        tout << std::endl
             << "Would you like to format the media? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip format.\n";
        ...
        tout << std::endl
             << "Choose a number Which media do you want to format ? \n";
        tout << "[-1] Cancel input\n";
        ...
        tout << std::endl
             << "All data will be deleted. Is it OK ? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip format.\n";
        ...
        {
            tout << std::endl
                 << "The Selected slot cannot be formatted.\n";
            return;
        }
    ...
    void CameraDevice::execute_movie_rec()
    {
        ...
        text input;
        tout << std::endl
             << "Operate the movie recording button ? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip .\n";
    ...
    void CameraDevice::set_custom_wb()
    {
        ...
        // Set, ExposureProgramMode property
        tout << std::endl
             << "Set the Exposure Program mode to P mode" << std::endl;
        SDK::CrDeviceProperty expromode;
        expromode.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_ExposureProgramMode);
        expromode.SetCurrentValue(SDK::CrExposureProgram::CrExposure_P_Auto);
        expromode.SetValueType(SDK::CrDataType::CrDataType_UInt16Array);
        ...
        tout << std::endl
             << "Set the White Balance to Custom1\n";
        ...
        tout << std::endl
             << "Set custom WB capture standby " << std::endl;
        ...
        {
            tout << std::endl
                 << "CustomWB Capture Standby FAILED\n";
            return;
        }
        ...
        text input;
        tout << std::endl
             << "Set custom WB capture standby cancel. Please enter something. " << std::endl;
        std::getline(tin, input);
    ...
    void CameraDevice::set_zoom_operation()
    {
        ...
        while (true)
        {
            ...
            {
                tout << std::endl
                     << "Set the value of zoom speed (Out-of-range value to Cancel):\n";
                tout << std::endl
                     << "input> ";
                std::getline(tin, input);
                text_stringstream ss(input);
    ...
    void CameraDevice::execute_camera_setting_reset()
    {
        ...
        text input;
        tout << std::endl
             << "The camera settings will be initialized! Are you sure you want to do this ? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip.\n";
    ...
    if (1 != m_prop.white_balance_tint.writable) {
        ...
        tout << std::endl
             << "Set a value within the White Balance Tint Range" << std::endl;
        tout << "input> ";
        std::getline(tin, input);
        text_stringstream ss(input);
        int num = 0;
    ...
    void CameraDevice::set_custom_wb_size_setting()
    {
        ...
        text input;
        tout << std::endl
             << "Would you like to set a new Custom WB Size Setting value? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip setting a new value.\n";
        ...
        tout << std::endl
             << "Choose a number set a new Custom WB Size Setting value:\n";
        tout << "[-1] Cancel input\n";
    ...
    void CameraDevice::set_time_shift_shooting()
    {
        ...
        text input;
        tout << std::endl
             << "Would you like to set a new TimeShift Shooting value? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip setting a new value.\n";
        ...
        tout << std::endl
             << "Choose a number set a new TimeShift Shooting value:\n";
        tout << "[-1] Cancel input\n";
    ...
    void CameraDevice::set_recording_setting_file_name()
    {
        ...
        size_t sLen = 0;
        if (0 < wstr.length())
            sLen = (wstr.length() + 1); // +1 = null terminate
    ...
    void CameraDevice::execute_pos_xy(CrInt16u code)
    {
        ...
        tout << std::endl
             << "Set the value of X (decimal)" << std::endl;
        ...
        tout << std::endl
             << "Set the value of Y (decimal)" << std::endl;
        ...
        tout << std::endl
             << "input X_Y = 0x" << std::hex << x_y << std::dec << '\n';
    ...
    void CameraDevice::execute_preset_focus()
    {
        ...
        tout << std::endl
             << "Save Zoom and Focus Position Enable Preset number: " << std::endl;
        ...
        tout << std::endl
             << "Load Zoom and Focus Position Enable Preset number: " << std::endl;
    ...
    void CameraDevice::execute_APS_C_or_Full()
    {
        ...
        text input;
        tout << std::endl
             << "Execute APS-C or FULL switching control of the camera ? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip.\n";
        ...
        {
            tout << std::endl
                 << "APS-C/Full switching is not executable.\n";
            return;
        }
        ...
        tout << std::endl
             << "Executed the switch between APS-C or Full.\n";
    }
    ...
    void CameraDevice::execute_movie_rec_toggle()
    {
        ...
        text input;
        tout << std::endl
             << "Operate the movie recording button toggle? (y/n): ";
        std::getline(tin, input);
        if (input != TEXT("y"))
        {
            tout << "Skip .\n";
    ...
    void CameraDevice::getFileNames(std::vector<text> &file_names)
    {
        ...
        if (dp != NULL)
        {
            while ((ep = readdir(dp)) != NULL)
            {
                ...
                {
                    text str(ep->d_name);
                    if (((ep->d_name[str.length() - 4] == '.') && (ep->d_name[str.length() - 3] == 'd') && (ep->d_name[str.length() - 2] == 'a') && (ep->d_name[str.length() - 1] == 't')) || ((ep->d_name[str.length() - 4] == '.') && (ep->d_name[str.length() - 3] == 'D') && (ep->d_name[str.length() - 2] == 'A') && (ep->d_name[str.length() - 1] == 'T')))
                    {
    ...
    void CameraDevice::change_live_view_enable()
    {
        ...
        tout << std::endl
             << "Are you sure you want to reverse EnableLiveView? (y/n) > ";
        text yesno;
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
    ...
    void CameraDevice::OnError(CrInt32u error)
    {
        ...
        if (!msg.empty())
        {
            ...
            if (SDK::CrError_Connect_SSH_ServerConnectFailed == error || SDK::CrError_Connect_SSH_InvalidParameter == error || SDK::CrError_Connect_SSH_ServerAuthenticationFailed == error || SDK::CrError_Connect_SSH_UserAuthenticationFailed == error || SDK::CrError_Connect_SSH_PortForwardFailed == error || SDK::CrError_Connect_SSH_GetFingerprintFailed == error)
            {
                m_fingerprint.clear();
                m_userPassword.clear();
            }
        ...
        if (prop_list && nprop > 0)
        {
            ...
            for (std::int32_t i = 0; i < nprop; ++i)
            {
                ...
                case SDK::CrDevicePropertyCode::CrDeviceProperty_BodySerialNumber:
                    if (nullptr != m_bodySerialNumberProp)
                        delete m_bodySerialNumberProp;
    ...
    void CameraDevice::getContentsList()
    {
        ...
        if (CR_SUCCEEDED(err))
        {
            ...
            while (1)
            {
                ...
                text input;
                tout << std::endl
                     << "Select the number of the contents you want to download:";
                tout << std::endl
                     << "(Returns to the previous menu for invalid numbers)" << std::endl
                     << std::endl;
                tout << std::endl
                     << "input> ";
                std::getline(tin, input);
                ...
                else
                {
                    while (1)
                    {
                        ...
                        text input;
                        tout << std::endl
                             << "Select the number of the content size you want to download:";
                        tout << std::endl
                             << "[-1] Cancel input";
                        tout << std::endl
                             << "[1] Original";
    ...
    void CameraDevice::pullContents(SDK::CrContentHandle content)
    {
        ...
        if (SDK::CrError_None != err)
        {
            ...
            if (!msg.empty())
            {
                // output is 2 line
                tout << std::endl
                     << msg.data() << ", handle=" << std::hex << content << std::dec << std::endl;
                tout << m_info->GetModel() << " (" << id.data() << ")" << std::endl;
            }
        }
    }
    ...
    void CameraDevice::execute_request_displaystringlist()
    {
        ...
        while (1)
        {
            ...
            if (0 == listName.compare(TEXT("end")))
                break;
    ...
    void CameraDevice::execute_get_displaystringlist()
    {
        if (m_dispStrTypeList.size() > 0)
        {
            ...
            if (selected_index == 0)
                type = SDK::CrDisplayStringType_AllList;
            else
                type = m_dispStrTypeList[selected_index - 1];
    ...
    void CameraDevice::set_focus_position_setting()
    {
        ...
        tout << std::endl
             << "Set a value within the Focus Position Setting (hex)" << std::endl;
        tout << "[-1] Cancel input\n"
             << std::endl;
        tout << "input> ";
    ...
    bool CameraDevice::execute_focus_position_cancel()
    {
        text input;
        tout << std::endl
             << "Do you want to continue with Setting Focus Position?" << std::endl;
        ...
        if (m_prop.focus_driving_status.current == SDK::CrFocusDrivingStatus::CrFocusDrivingStatus_Driving)
        {
            ...
            tout << std::endl
                 << "Execute Cancel Focus Position Setting\n";
        }
        else
        {
    ...
    void CameraDevice::execute_get_zoom_and_focus_preset()
    {
        ...
        if (CR_SUCCEEDED(ret) && 0 < presetNums)
        {
            for (CrInt32u i = 0; i < presetNums; i++)
            {
                ...
                tout << "     ZoomOnly Value         : " << (int)ZoomAndFocusPreset[loadNum].zoomOnlyValue << std::endl
                     << std::endl;
            }
    ...
    bool CameraDevice::execute_zoom_position_cancel()
    {
        text input;
        tout << std::endl
             << "Do you want to continue with Setting Zoom Position?\n";
        tout << "[0] Continue\n";
        tout << "[1] Cancel\n";
        tout << "input> ";
        std::getline(tin, input);
    ...
    void CameraDevice::printOtherInfo()
    {
        ...
        text modelName = TEXT("-");
        if (nullptr != m_bodySerialNumberProp)
            bodySerial = getCurrentStr(m_bodySerialNumberProp);
        if (nullptr != m_lensModelNameProp)
            lensName = getCurrentStr(m_lensModelNameProp);
        if (nullptr != m_modelNameProp)
            modelName = getCurrentStr(m_modelNameProp);
    ...
    {
        if (nullptr == prop)
            return TEXT("target pointer is null");

        if (SCRSDK::CrDataType_STR != prop->GetValueType())
            return TEXT("target is not CrDataType_STR");
    ...
    void CameraDevice::setMoviePlaybackSetting()
    {
        ...
        if (hostIp.length() == 0)
        {
            ...
            tout << "set default. 192.168.0.1" << std::endl
                 << std::endl;
        }
    ...
    void CameraDevice::cancel_upload_firmware()
    {
        ...
        tout << std::endl
             << "Are you sure you want to cancel upload firmware ? (y/n) > ";
        text yesno;
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
    ...
    void CameraDevice::execute_update_firmware()
    {
        ...
        tout << std::endl
             << "Are you sure you want to execute firmware update? (y/n) > ";
        text yesno;
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
    ...
    void CameraDevice::get_firmware_updater_info()
    {
        ...
        tout << std::endl
             << "Are you sure you want to request firmware updater info? (y/n) > ";
        text yesno;
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
    ...
    void CameraDevice::set_debug_mode()
    {
        ...
        tout << std::endl
             << "Are you sure you want to set debug mode? (y/n) > ";
        text yesno;
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
    ...
    void CameraDevice::set_timezone_setting()
    {
        ...
        // DateTime
        tout << std::endl
             << "Are you sure you want to set DateTime? (y/n) > ";
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
        ...
        // Area
        tout << std::endl
             << "Are you sure you want to set Area? (y/n) > ";
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
        ...
        // Daylight Savings
        tout << std::endl
             << "Are you sure you want to set Daylight Savings? (y/n) > ";
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
    ...
    void CameraDevice::control_ptzf()
    {
        ...
        {
            tout << std::endl
                 << "Are you sure you want to set Pan Settings? (y/n) > ";
            text yesno;
            std::getline(cli::tin, yesno);
            if (yesno != TEXT("y"))
            {
        ...
        {
            tout << std::endl
                 << "Are you sure you want to set Tilt Settings? (y/n) > ";
            text yesno;
            std::getline(cli::tin, yesno);
            if (yesno != TEXT("y"))
            {
    ...
    void CameraDevice::set_ptzf_preset()
    {
        ...
        tout << std::endl
             << "Are you sure you want to set thumbnail? (y/n) > ";
        text yesno;
        std::getline(cli::tin, yesno);
        if (yesno != TEXT("y"))
        {
